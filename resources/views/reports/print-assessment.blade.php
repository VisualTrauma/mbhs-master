<!DOCTYPE html>
<html>
    <head>
        <meta charset="utf-8">
        <title>Assessment Form | MBHS</title>
        <link rel="stylesheet" type="text/css" id="theme" href="{{ URL::to('css/theme-default.css') }}"/>
    </head>
    <body>
        <div class="row" >

            <div class="col-md-12">
                <div class="panel panel-default">

                    <div class="panel-body">
                        <table>
                            <tbody>
                                <tr>
                                    <td>
                                        <p style="text-align: left;"><img src="data:image/jpeg;base64," data-filename="mbhs_logo.jpg" style="width: 100px; float: none;"></p>
                                    </td>
                                    <td>
                                        <h3 style="text-align: left; margin-left: 15px">Muntinlupa Business High School</h3>
                                        <p style="text-align: left; line-height:5px; margin-left: 15px">Espeleta St. Buli, Muntinlupa City</p>
                                        <p style="text-align: left; line-height:5px; margin-left: 15px">Telephone No: 850-9479</p>
                                    </td>
                                </tr>
                            </tbody>
                        </table>
                        <hr style="margin-top:-20px;">
                        <h3 style="text-align: center; margin-top: 40px;">Assessment Form</h3>
                    </div>

                    <h6 style="text-align: left; margin-top: 20px; margin-left: 20px;">Student Information</h6>

                    <div class="panel-body" style="margin-top:-10px;">
                        <table class="table table-bordered">
                            <tbody>
                                <tr>
                                    <td width="30%" height="50%">
                                        <p style="text-align: left; margin-left: 15px">LRN: </p>
                                    </td>
                                    <td height="50%">
                                        <p style="text-align: left; margin-left: 15px">{{ $enrollment->student->lrn }}</p>
                                    </td>
                                </tr>
                                <tr>
                                    <td width="30%" height="50%">
                                        <p style="text-align: left; margin-left: 15px">Reg. Code: </p>
                                    </td>
                                    <td height="50%">
                                        <p style="text-align: left; margin-left: 15px">{{ $enrollment->registration_code }}</p>
                                    </td>
                                </tr>
                                <tr>
                                    <td width="30%">
                                        <p style="text-align: left; margin-left: 15px">Name: </p>
                                    </td>
                                    <td>
                                        <p style="text-align: left; margin-left: 15px">{{ $enrollment->student->last_name . ', ' . $enrollment->student->last_name . ' ' . $enrollment->student->middle_name}}</p>
                                    </td>
                                </tr>
                                <tr>
                                    <td width="30%">
                                        <p style="text-align: left; margin-left: 15px">Address: </p>
                                    </td>
                                    <td>
                                        <p style="text-align: left; margin-left: 15px">{{ $enrollment->student->address }}</p>
                                    </td>
                                </tr>
                            </tbody>
                        </table>
                    </div>

                    <h6 style="text-align: left; margin-top: 20px; margin-left: 20px;">Enrollment Information</h6>

                    <div class="panel-body" style="margin-top:-10px;">
                        <table class="table table-bordered">
                            <tbody>
                                <tr>
                                    <td width="30%">
                                        <p style="text-align: left; margin-left: 15px">Grade Level: </p>
                                    </td>
                                    <td>
                                        <p style="text-align: left; margin-left: 15px">{{ $enrollment->grade_level }}</p>
                                    </td>
                                </tr>
                                <tr>
                                    <td width="30%">
                                        <p style="text-align: left; margin-left: 15px">General Avg.: </p>
                                    </td>
                                    <td>
                                        <p style="text-align: left; margin-left: 15px">{{ $enrollment->general_average }}</p>
                                    </td>
                                </tr>
                                <tr>
                                    <td width="30%">
                                        <p style="text-align: left; margin-left: 15px">Section: </p>
                                    </td>
                                    <td>
                                        <p style="text-align: left; margin-left: 15px">{{ $enrollment->section->name }}</p>
                                    </td>
                                </tr>
                                <tr>
                                    <td width="30%">
                                        <p style="text-align: left; margin-left: 15px">Room: </p>
                                    </td>
                                    <td>
                                        <p style="text-align: left; margin-left: 15px">{{ $enrollment->section->room }}</p>
                                    </td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
                </div>
            </div>

            <div class="col-md-3" style="position: relative;">
                <div id="tocify"></div>
            </div>

        </div>

        <script type="text/javascript" src="{{ URL::to('js/plugins/jquery/jquery.min.js') }}"></script>
        <script type="text/javascript" src="{{ URL::to('js/plugins/jquery/jquery-ui.min.js') }}"></script>
        <script type="text/javascript">
            $(function() {
                window.print();
            });
        </script>
    </body>
</html>
